...
<header class="flex flex-wrap sm:justify-start sm:flex-nowrap z-50 w-full bg-white text-sm py-4 dark:bg-gray-800">
  <nav class="max-w-[85rem] w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between" aria-label="Global">
    ...
    <div id="navbar-with-collapse" class="hidden basis-full grow sm:block">
      <div class="flex flex-col gap-8 mt-5 text-lg sm:flex-row sm:items-center sm:justify-end sm:mt-0 sm:ps-5 ">
        <div v-for="link in ConfSite.menu">
        <a href="" class=""> {{ link.name }} </a>
    </div>
    <button class="bg-red-400 text-white px-8 py-2 rounded-lg font-bold text-base">Faites un don</button>
      </div>
    </div>
